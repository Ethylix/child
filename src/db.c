...
void loadanopedb()
{
    ...
    while((row = mysql_fetch_row(result))) {
        ...
        if (acclvl < CHLEV_OWNER)
            AddUserToChannel(uptr,chan_ptr,acclvl);
    }
...
void loadchandb()
{
    ...
    while ((row = mysql_fetch_row(result))) {
        ...
        if (acclvl < CHLEV_OWNER && !IsMask(accname)) {
            cflag = AddUserToChannel(user_ptr,chan_ptr,acclvl);
...
void savechandb()
{
    ...
    LIST_FOREACH_ALL(cflag_list, cflag) {
        ...
        snprintf(tmp,1024,"INSERT INTO `child_chan_access` VALUES ('%s','%s',%d,%d,%d)",chname,strtosql(nick,cflag->nick,NICKLEN),cflag->flags,cflag->automode,cflag->suspended);
        mysql_query(&mysql,tmp);
    }
