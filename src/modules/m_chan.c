...
void chan_access (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (!Strcmp(arg4,"add")) {
        ...
        AddUserToChannel(uptr2,chptr,newlevel);
...
void chan_op (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (GetFlag(uptr,chptr) < me.chlev_op && !IsFounder(uptr,chptr)) {
        NoticeToUser(nptr,"Access denied");
        return;
    }   
...
void chan_voice (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (((GetFlag(uptr,chptr) < me.chlev_voice && (!arg4 || *arg4 == '\0')) || (GetFlag(uptr,chptr) < me.chlev_halfop && arg4 && *arg4)) && !IsFounder(uptr,chptr)) {
        NoticeToUser(nptr,"Access denied");
        return;
    }
...
void chan_halfop (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (((GetFlag(uptr,chptr) < me.chlev_halfop && (!arg4 || *arg4 == '\0')) || (GetFlag(uptr,chptr) < me.chlev_op && arg4 && *arg4)) && !IsFounder(uptr,chptr)) {
        NoticeToUser(nptr,"Access denied"); 
        return;
    }   
...
void chan_invite (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (GetFlag(uptr,chptr) < me.chlev_invite && !IsFounder(uptr,chptr)) {
        NoticeToUser(nptr,"Access denied");
        return;
    }   
...
void chan_set_nojoin (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (GetFlag(uptr,chptr) < me.chlev_sadmin && !IsFounder(uptr,chptr)) {
        NoticeToUser(nptr,"Access denied");
        return;
    }
...
void chan_set_avoice (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (GetFlag(uptr,chptr) < me.chlev_admin && !IsFounder(uptr,chptr)) {
        NoticeToUser(nptr,"Access denied");
        return;
    }
...
void chan_set_enftopic (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (GetFlag(uptr,chptr) < me.chlev_admin) {
        NoticeToUser(nptr, "Access denied");
        return;
    }
...
void chan_set_founder (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    AddUserToChannel(uptr2,chptr,CHLEV_OWNER);
...
void chan_info (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (HasOption(chptr, COPT_PRIVATE) && GetFlag(uptr,chptr) < 1 && !IsFounder(uptr,chptr) && (uptr->level < me.level_oper || !IsOper(nptr))) {
...
void chan_kick (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (GetFlag(uptr,chptr) < me.chlev_halfop && !IsFounder(uptr,chptr)) {
        NoticeToUser(nptr,"Access denied");
        return;
    }
...
void chan_banlist (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (GetFlag(uptr,chptr) < me.chlev_halfop) {
        NoticeToUser(nptr, "Access denied.");
        return;
    }
...
void chan_topic (Nick *nptr, User *uptr, Chan *chptr, char *all)
{
    ...
    if (GetFlag(uptr, chptr) < me.chlev_op) {
        NoticeToUser(nptr, "Access denied.");
        return;
    }
    ...
    if (!topic || *topic == '\0') {
        ...
        SendRaw(":%s TOPIC %s :", whatbot(ch), ch);
        NoticeToUser(nptr, "Topic cleared.");
    } else {
        strncpy(chptr->topic, topic, TOPICLEN);
        SendRaw(":%s TOPIC %s :%s", whatbot(ch), ch, topic);
        NoticeToUser(nptr, "Done.");
    }
}
