...
void sync_cflag (Cflag *cflag, Nick *nptr)
{
    ...
    if (!IsMember(uptr->nick, cflag->channel))
        return;
    ...
    if (cflag->automode == CFLAG_AUTO_OFF || cflag->suspended == 1)
        return;
    ...
    if ((cflag->flags == CHLEV_OWNER || cflag->flags == CHLEV_COOWNER)) {
        ...
        SetStatus(nptr, cflag->channel, cflag->flags, 0, bot);
}
